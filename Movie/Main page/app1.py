...
def adminupdate():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO MOVIES(MOV_TITLE,MOV_YEAR,MOV_LANG,DIR_ID) VALUES (%s,%s,%s,%s)",(mov_title,mov_year,mov_lang,dir_id))
        mysql.connection.commit()
        cur.close()
        val = "Updation successful! Thank you for signing up, your details include: Movie Title: {0}, Movie Year: {1}, Movie Language: {2}, Director ID: {3}".format(mov_title,mov_year,mov_lang,dir_id)
        return redirect(url_for('logout'))
